...
function LoopThreeTexts(
  ...
  const holdDuration  = 2.2;  // seconds each text stays visible
  const gapDuration   = 0.10; // brief blank gap between exit and enter

  const maxExitChars1 = Text1.chars.length;
  const maxExitChars2 = Text2.chars.length;
  const maxExitChars3 = Text3.chars.length;

  const totalExitTime1 = exitDuration + maxExitChars1 * 0.02;
  const totalExitTime2 = exitDuration + maxExitChars2 * 0.02;
  const totalExitTime3 = exitDuration + maxExitChars3 * 0.02;
  ...
  // ── Exit Text 1 ──────
  tl.to(Text1.chars, { y: -80, opacity: 0, duration: exitDuration, ease: "power2.in", stagger: 0.02 }, t1)
    .set(Text2.chars, { y: 80, opacity: 0, immediateRender: false }, t2 - 0.01)

  // ── Enter Text 2 ──────
    .to(Text2.chars, { y: 0, opacity: 1, duration: enterDuration, ease: "power3.out", stagger: 0.025 }, t2)

  // ── Exit Text 2 ──────
    .to(Text2.chars, { y: -80, opacity: 0, duration: exitDuration, ease: "power2.in", stagger: 0.02 }, t3)
    .set(Text3.chars, { y: 80, opacity: 0, immediateRender: false }, t4 - 0.01)

  // ── Enter Text 3 ──────
    .to(Text3.chars, { y: 0, opacity: 1, duration: enterDuration, ease: "power3.out", stagger: 0.025 }, t4)

  // ── Exit Text 3 ──────
    .to(Text3.chars, { y: -80, opacity: 0, duration: exitDuration, ease: "power2.in", stagger: 0.02 }, t5)
    .set(Text1.chars, { y: 80, opacity: 0, immediateRender: false }, t6 - 0.01)

  // ── Enter Text 1 ──────
    .to(Text1.chars, { y: 0, opacity: 1, duration: enterDuration, ease: "power3.out", stagger: 0.025 }, t6);
}
